...
const Datatableditolak = ({itemsEmail, itemsUserid}) =>{
  ...
  async function onSubmit(values){
    ...
    try {
      getTicketDitolak(values.searchticket); 
    } catch(err){
      console.log(err)
    }
  } 

  async function getTicketDitolak(searchDash){
    const api = `${process.env.NEXT_PUBLIC_API_SITE}/api/ticket?status=3&user_id=${tknemail.userid}&search=${searchDash}`
...
export default Datatableditolak;