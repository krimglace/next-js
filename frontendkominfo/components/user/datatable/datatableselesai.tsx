...
const Datatableselesai = ({itemsEmail, itemsUserid}) =>{
  ...
  async function onSubmit(values){
    ...
    try {
      getTicketSelesai(values.searchticket); 
    } catch(err){
      console.log(err)
    }
  } 

  async function getTicketSelesai(searchDash){
    const api = `${process.env.NEXT_PUBLIC_API_SITE}/api/ticket?status=2&user_id=${tknemail.userid}&search=${searchDash}`
...
export default Datatableselesai;