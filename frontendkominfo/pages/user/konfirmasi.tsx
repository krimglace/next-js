...
import Konfirmasitable from '../../components/user/datatable/datatablekonfirmasi'
...
const Konfirmasi = ({email, iduser}) => {
  ...
  function pullData(){
    document.querySelector('#konfirmasi').classList.add('active')
  }
  useEffect(() => {
    pullData()
  }, [])
     ...
     <Header itemsEmail={tknuser.emailuser} itemsUserid={tknuser.iduseruser} />
      <Sidebar itemsEmail={tknuser.emailuser} itemsUserid={tknuser.iduseruser} />
        ...
        <div className="bg-light p-lg-4 p-md-4 p-3">
          <h5 className="float-md-start float-lg-start"><strong>Permohonan Terkonfirmasi</strong></h5>
          <small className="float-md-end float-lg-end"><Link href={`/user/dashboard`} className="text-primary">Dashboard /</Link> <Link href="#" className="text-primary">Permohonan / </Link>Konfirmasi</small>
          <div className="clearfix" />
        </div>
        <br />
        <Konfirmasitable itemsEmail={tknuser.emailuser} itemsUserid={tknuser.iduseruser}/>
        </div>
      <Footer />
      <Boots />
    </div>
